function loadBanamaData( banamaData ){
    //	-----------------------------------------------------------------------------
    //	Load the world geo data json, per country	

	//	iterate through each set of country pins
	for ( var i in banamaData.countries ) {										
		var banama = banamaData.countries[i];	
		...
		banama.companies = banama["companies"];
		banama.intermediary =  banama["intermediary"];
		banama.officer =  banama["officer"];

		banama.countryCode = i;
		banama.countryName = countryLookup[i];			        
	
		banamabigData[banama.countryName] = banama;	
	}		
